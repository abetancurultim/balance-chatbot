...
import { z } from "zod";
import { contactCustomerService, saveClientData } from '../utils/functions.js';
...
export const saveClientDataTool = tool(
  async ({ name, phone, email, message }: { name: string, phone: string, email: string, message: string }) => {
